...
\begin{subdefinition}
Suma Minkowskiego
\\Sum± Minkowskiego nazywamy dzia³anie okre¶lone na rodzinie wszystkich podzbiorów danej przestrzeni liniowej $X$ wzorem:$$A+B=\{a+b:a \in A,b \in B\},$$gdzie $A$ i $B$ s± podzbiorami przestrzeni liniowej $X$.
\end{subdefinition}
~\\Powy¿sza definicja ma sens dla dowolnego zbioru $X$ z okre¶lonym dzia³aniem dodawania.
~\\W przypadku zbiorów dwuwymiarowych $P, Q \in \mathbb{R}^2$, bêd±cych podzbiorami p³aszczyzny, sumê Minkowskiego tych zbiorów mo¿na interpretowaæ jako sumê wektorow± wektorów $p \in P$ i $q \in Q$, czyli gdy $p=(p_{x},p_{y})$ oraz $q=(q_{x},q_{y})$, to $p + q = (p_{x} + q_{x},p_{y} + q_{y})$.

~\\Na poni¿szym obrazku zaprezentowano przyk³ad sumy Minkowskiego dla dwóch podzbiorów p³aszczyzny $P, Q \in \mathbb{R}^2$.\\$P = \{(1,0), (0,1), (0,-1)\}$\\$Q = \{(0,0), (1,1), (1,-1)\}$\\$P + Q = \{(1,0), (2,1), (2,-1), (0,1), (1,2), (0,-1), (1,-2)\}$.
...
~\\W kolejnym kroku nale¿y wybraæ dowolny punkt znajduj±cy siê wewn±trz lub na brzegu jednego z wielok±tów. W szczególno¶ci mo¿e byæ to jeden z wierzcho³ków wielok±ta. Wybierzmy jeden z wierzcho³ków wielok±ta $B$.
...
~\\Po dokonaniu wyboru punktu rozpoczyna siê g³ówna faza algorytmu. W pierwszym kroku rozwa¿ane bêd± trójk±ty $A1$ i $B1$. Na pocz±tek nastêpuje translacja wybranego w poprzednim kroku punktu do jednego z wierzcho³ków trójk±ta $A1$. Po wykonaniu tej operacji o taki sam wektor nale¿y przesun±æ wszystkie wierzcho³ki trójk±ta $B1$. Efekt tej operacji zaprezentowano na obrazku poni¿ej.

~\begin{center}
	\includegraphics[scale=0.6]{MinkowskiSumExampleStep3}\\
	\textit{\textbf{Obrazek 4.3.1.1}. Translacja wybranego punktu i trójk±ta $B1$}
\end{center}

~\\Po wykonaniu opisanej wy¿ej translacji nale¿y zapisaæ (np. na li¶cie) wspó³rzêdne wierzcho³ków przesuniêtego trójk±ta $B1$, a nastêpnie przeprowadziæ translacjê wybranego wcze¶niej punktu do kolejnego wierzcho³ka trójk±ta $A1$ oraz przesun±æ o taki sam wektor wszystkie wierzcho³ki trójk±ta $B1$. Ponownie nale¿y zapisaæ nowe wspó³rzêdne wierzcho³ków trójk±ta $B1$ oraz powtórzyæ operacjê translacji dla ostatniego wierzcho³ka trójk±ta $A1$. Na koniec, po raz kolejny nale¿y zapisaæ wspó³rzêdne wierzcho³ków przesuniêtego trójk±ta $B1$.

~\\W nastêpnym kroku nale¿y wyznaczyæ otoczkê wypuk³± zbioru punktów zapisanych w poprzednim kroku. Efekt wyznaczania otoczki wypuk³ej zbioru tych punktów oraz odpowiednio przesuniêty trójk±t $B1$ (do ka¿dego z wierzcho³ków trójk±ta $A1$) prezentuje poni¿szy obrazek.

~\begin{center}
	\includegraphics[scale=0.6]{MinkowskiSumExampleStep4}\\
	\textit{\textbf{Obrazek 4.3.1.1}. Otoczka wypuk³a (kolor zielony) zapisanego zbioru punktów oraz przesuniêty (do ka¿dego z wierzcho³ków $A1$) trójk±t $B1$ (kolor niebieski)}
\end{center}

~\\W kolejnym kroku nale¿y powtórzyæ opisane wy¿ej operacje dla pary trójk±tów $A1$ i $B2$. Ponownie wyznaczony zostanie zbiór punktów, dla którego nale¿y wyznaczyæ otoczkê wypuk³±. Efekt zaprezentowano na obrazku poni¿ej.

~\begin{center}
	\includegraphics[scale=0.6]{MinkowskiSumExampleStep5}\\
	\textit{\textbf{Obrazek 4.3.1.1}. Otoczka wypuk³a (kolor zielony) zapisanego zbioru punktów dla trójk±tów $A1$ i $B2$}
\end{center}
...
~\begin{center}
	...
	\textit{\textbf{Obrazek 4.3.1.1}. Efekt z³±czenia wyznaczonych w poprzednich krokach otoczek wypuk³ych}
\end{center}

~\\Po wykonaniu powy¿szych kroków powsta³ nowy wielok±t (kolor pomarañczowy) i przetwarzanie trójk±ta $A1$ zosta³o zakoñczone. Nastêpnie wszystkie operacje wykonane dla trójk±ta $A1$ nale¿y powtórzyæ dla trójk±ta $A2$. Po zakoñczeniu przetwarzania trójk±ta $A2$ przetwarzany jest trójk±t $A3$, itd. Operacja zakoñczy siê po przetworzeniu wszystkich trójk±tów wchodz±cych w sk³ad wielok±ta $A$. Efekt przetworzenia wszystkich trójk±tów zaprezentowano na poni¿szym obrazku.
...
~\\Ostatnim krokiem prowadz±cym do uzyskania sumy Minkowskiego jest z³±czenie wszystkich nowo powsta³ych wielok±tów (kolory pomarañczowy czerwony, zielony i niebieski). Efektem tej operacji jest wyznaczona suma Minkowskiego wielok±tów $A$ i $B$.
...
~\\Aby wyznaczyæ z³o¿ono¶æ algorytmu obliczania sumy Minkowskiego dwóch wielok±tów przeanalizujmy z³o¿ono¶æ wystêpuj±cych w nim operacji. Przyjmijmy nastêpuj±ce oznaczenia:
\\$n$ - liczba wierzcho³ków pierwszego wielok±ta,
\\$m$ - liczba wierzcho³ków drugiego wielok±ta.
\\Ka¿dy wielok±t jest reprezentowany w postaci listy punktów.

~\\W pierwszym kroku dzia³ania algorytmu znajdowana jest triangulacja ka¿dego z wielok±tów w czasie $O(p^2)$, gdzie $p$ jest liczb± wierzcho³ków tworz±cych dany wielok±t. Nastêpnie nastêpuje przetwarzanie parami trójk±tów z pierwszego wielok±ta z trójk±tami drugiego wielok±ta. Dla ka¿dej pary trójk±tów trzykrotnie liczymy translacjê wszystkich wierzcho³ków jednego z trójk±tów. Przyjmijmy ¿e bêdzie to trójk±t nale¿±cy do drugiego wielok±ta. Operacja translacji wykonywana jest w czasie sta³ym - $O(1)$. Ponadto na koniec przetwarzania ka¿dej z par liczona jest otoczka wypuk³a wyznaczonych przez algorytm punktów.

~\\Z³o¿ono¶æ ¶rednia operacji wyznaczania otoczki wypuk³ej wynosi $O(klog(k))$, gdzie $k$ jest liczb± punktów, dla których ta otoczka jest wyznaczana. Jednak w naszym przypadku mo¿na uznaæ, ¿e operacja ta jest wykonywana w czasie $O(1)$, gdy¿ niezale¿nie od liczby wierzcho³ków tworz±cych wielok±ty dla których liczona jest suma Minkowskiego, otoczka bêdzie zawsze liczona dla $k = 9$. 

~\\Po zakoñczeniu przetwarzania jednego trójk±ta nale¿±cego do pierwszego wielok±ta z wszystkimi trójk±tami drugiego wielok±ta wykonywana jest operacja z³±czania powsta³ych w tym procesie otoczek wypuk³ych. Operacja ta wykonywana jest w czasie $O(\ell^2)$, gdzie $\ell$ jest ³±czn± liczb± wierzcho³ków wchodz±cych w sk³ad wyznaczonych otoczek wypuk³ych. Operacja z³±czania wielok±tów jest wykonywana tak¿e na koñcu dzia³ania algorytmu, po przetworzeniu wszystkich par.

~\\Z³o¿ono¶æ ca³ego algorytmu mo¿na oszacowaæ poprzez oszacowanie z³o¿ono¶ci jego operacji dominuj±cej. W tym wypadku operacj± t± jest operacja, w której parami przetwarzane s± wszystkie trójk±ty pierwszego wielok±ta z trójk±tami drugiego wielok±ta. Jej z³o¿ono¶æ mo¿na oszacowaæ w nastêpuj±cy sposób:$$(n - 2) \cdot (m - 2) + (n - 2) \cdot (9 \cdot (m - 2))^2.$$\\Liczby $n - 2$ oraz $m - 2$ oznaczaj± liczbê trójk±tów na jakie zosta³ podzielony odpowiednio pierwszy i drugi wielok±t. Pierwszy sk³adnik powy¿szej sumy opisuje wiêc z³o¿ono¶æ przetworzenia parami wszystkich trójk±tów i wyznaczenia kolejnych otoczek wypuk³ych. Drugi sk³adnik opisuje z³o¿ono¶æ operacji z³±czania otoczek wypuk³ych, która jest wykonywana dla ka¿dego trójk±ta pierwszego wielok±ta. Sk³adnik ten ma tak± postaæ, gdy¿ $n - 2$ to oczywi¶cie liczba trójk±tów na jakie mo¿na podzieliæ pierwszy wielok±t, a $9 \cdot (m-2)$ to z góry oszacowana liczba wierzcho³ków wchodz±cych w sk³ad ka¿dej otoczki wypuk³ej - jedna otoczka mog³aby mieæ maksymalnie 9 wierzcho³ków, a dla jednego trójk±ta pierwszego wielok±ta powstanie $m - 2$ takich otoczek, gdy¿ na tyle trójk±tów mo¿na podzielic drugi wielok±t. Ogóln± z³o¿ono¶æ algorytmu obliczaj±cego sumê Minkowskiego mo¿na wiêc oszacowaæ jako $O(n \cdot m^2)$.
